...
- Для всех строковых типов данных мы используем тип `text`. Это удобно в плане поддержки.
- Все файлы у нас хранятся в служебной таблице `projectdb_web`, в таблицах данной БД мы записываем uuid этой записи.
...
В поле test_config может записываться пользовательская конфигурация теста, например, количество вопросов для показа во время прохождения, брать ли случайные вопросы и т.п. Специальных настроек может не быть, сервис предполагает наличие дефолтного конфига, поэтому это поле не обязательное.
